...
train<-fread("train.csv",drop=c("QuoteNumber","Original_Quote_Date",
      "PersonalField16","PersonalField17","PersonalField18","PersonalField19"),na.strings = c("NA","NAN",""," "))
train<-as.data.frame(train)
test<-fread("test.csv",drop=c("QuoteNumber","Original_Quote_Date",
      "PersonalField16","PersonalField17","PersonalField18","PersonalField19"),na.strings = c("NA","NAN",""," "))
...
rm(posSeq,negSeq,pos,neg,train,isNum,nzv,i)
preObj<-preProcess(training[,-1],method = c("knnImpute","scale","center"))
training[,-1]<-predict(preObj,training[,-1])

model<-randomForest(y=training[,1],x=training[,-1],ntree = 500,importance=TRUE)
